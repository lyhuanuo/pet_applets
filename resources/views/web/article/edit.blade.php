...
<form class="layui-form layui-form-pane1" action="" lay-filter="first">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" name="title" lay-verify="required|title" lay-reqText="标题不能为空" required
                   placeholder="请输入标题" autocomplete="off" class="layui-input" value="{{$articleInfo['title']}}">
        </div>
    </div>
    ...
    <div class="layui-form-item" pane>
        ...
        <div class="layui-input-block">
            <input type="radio" name="type" value="1" title="操作指南" @if ($articleInfo['type'] == 1) checked @endif>
            <input type="radio" name="type" value="2" title="用户协议" @if ($articleInfo['type'] == 2) checked @endif >
        </div>
    </div>
...
<script>
    layui.use(['form', 'jquery','upload','layedit'], function () {
        ...
        form.on('submit(saveBtn)', function (data) {
            ...
            $.ajax({
                type : "post",
                url : "{{route('article/edit')}}/"+data.id,
                ...
                success : function(data) {
                    ...
                    if (data.code == 0) {
                        setTimeout(function(){
                            var index = parent.layer.getFrameIndex('article_edit');
